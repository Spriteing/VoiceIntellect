...
object CacheUtil {
     ...
     * 保存当前房间的id
     */
    fun setCurRoom(roomEntity: RoomEntity) {
        val kv = MMKV.mmkvWithID("cachke")
        kv.encode("room", Gson().toJson(roomEntity))
    }

    /**
     * 获取房间的id
     */
    fun getCurRoom(): RoomEntity? {
        val kv = MMKV.mmkvWithID("cachke")

        val roomEntity = kv.decodeString("room")
        return if (TextUtils.isEmpty(roomEntity)) {
            null
        } else {
            Gson().fromJson(roomEntity, RoomEntity::class.java)
        }
    }
}